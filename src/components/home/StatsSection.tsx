...
export function StatsSection() {
  return (
    <section className="relative py-24 md:py-36 overflow-hidden" style={{ backgroundColor: '#0A0A0A' }}>
      {/* Subtle ambient glow */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3">
          {STATS.map(({ value, suffix, label }, i) => (
            <div
              key={label}
              className={`py-10 md:py-0 ${i < STATS.length - 1 ? 'border-b md:border-b-0 md:border-r border-white/[0.06]' : ''} ${i > 0 ? 'md:pl-16' : ''} ${i < STATS.length - 1 ? 'md:pr-16' : ''}`}
            >
